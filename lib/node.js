 ...
 */
const unwrap = node => node === null || node.node === null ? node : node.node;
...
const wrapNode = (node) => {
    ...
    return {
        ...
        parent: () => wrapNode(node.parentNode),
        // children

        // get and set element attributes
        getType: () => node.tagName.toLowerCase().trim(),
        getKind: () => {
        ...
        getContent: () => node.textContent,
        setContent: val => node.textContent = val,
        getHTML: () => node.innerHTML,
        setHTML: val => node.innerHTML = val,
